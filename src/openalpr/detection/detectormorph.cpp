...
namespace alpr {
  ...
  std::vector<cv::Rect> DetectorMorph::find_plates(cv::Mat frame_gray, cv::Size min_plate_size, cv::Size max_plate_size)
  {
	...
	morphologyEx(frame_gray, img_open, MorphTypes::MORPH_OPEN, element, cv::Point(-1, -1));
    ...
    morphologyEx(img_threshold, img_open2, MorphTypes::MORPH_OPEN, diamond, cv::Point(-1, -1));
    Mat rectElement = getStructuringElement(cv::MORPH_RECT, Size(13, 4));
    morphologyEx(img_open2, img_threshold, MorphTypes::MORPH_CLOSE, rectElement, cv::Point(-1, -1));
